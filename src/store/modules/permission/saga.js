...
import { dispatchSetRoutes, setRoutes } from './action'
import { dispatchActiveRouteMatch } from '../app/action'
...
function *sagaSetRoutes({ payload: {roles, path}}) {
  let accessedRoutes;
  ...
  const routes = constantRoutes.concat(expandAccessedRoutes);
  yield put(setRoutes({
    routes,
    accessedRoutes,
    expandAccessedRoutes
  }))
  yield put(dispatchActiveRouteMatch({ routes, path }))
}

function* watchCommon() {
  yield takeEvery(dispatchSetRoutes, sagaSetRoutes)
}
